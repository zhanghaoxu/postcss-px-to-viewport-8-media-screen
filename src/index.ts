...
import type { MediaScreenOption, OptionType, ParentExtendType, RuleType } from './types';
import {
  blacklistedSelector,
  createPxReplace,
  declarationExists,
  ...
  return {
    ...
    Once(css: Root, { result }: { result: any }) {
      ...
      css.walkRules((rule: RuleType) => {
        ...
        const isAtRule = rule.parent?.params;
        ...
        if (Array.isArray(opts.mediaScreen) && opts.mediaScreen.length > 0 && rule.parent?.params) {
          opts.mediaScreen.forEach((option: MediaScreenOption) => {
            if (validateScreen(option.screen, rule.parent?.params)) {
              const mediaOption = { ...mediaOptionDefault, ...option }
              transformDecls({ ...opts, ...mediaOption })
            }
          })
        }
